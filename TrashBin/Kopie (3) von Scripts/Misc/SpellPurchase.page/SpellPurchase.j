//! runtextmacro BaseStruct("SpellPurchase", "SPELL_PURCHASE")
    ...
    static method Init takes nothing returns nothing
        ...
        //Act1
        call thistype.Create(ItemType.BLIZZARD, 5, Blizzard.THIS_SPELL)
        call thistype.Create(ItemType.CHILLY_BREATH, 5, ChillyBreath.THIS_SPELL)
        call thistype.Create(ItemType.FIREBALL, 5, Fireball.THIS_SPELL)
        call thistype.Create(ItemType.VIVID_METEOR, 5, VividMeteor.THIS_SPELL)

        //Act2
        call thistype.Create(ItemType.BARRIER, 5, Barrier.THIS_SPELL)
        call thistype.Create(ItemType.FLAME_TONGUE, 5, FlameTongue.THIS_SPELL)
        call thistype.Create(ItemType.HEAT_EXPLOSION, 5, HeatExplosion.THIS_SPELL)
        call thistype.Create(ItemType.ICE_BLOCK, 5, IceBlock.THIS_SPELL)
        call thistype.Create(ItemType.WARMTH_MAGNETISM, 5, WarmthMagnetism.THIS_SPELL)

        //Act3
        call thistype.Create(ItemType.FROZEN_STAR, 5, FrozenStar.THIS_SPELL)
        call thistype.Create(ItemType.GHOST_SWORD, 5, GhostSword.THIS_SPELL)
        call thistype.Create(ItemType.SEVERANCE, 5, Severance.THIS_SPELL)
        call thistype.Create(ItemType.SNOWY_SPHERE, 5, SnowySphere.THIS_SPELL)
