...
//! runtextmacro BaseStruct("Trigger", "TRIGGER")
    ...
    static method AddObjectInit takes integer tableKey, code c returns nothing
...
call SaveInteger(thistype.INIT_TABLE, tableKey, 0, LoadInteger(thistype.INIT_TABLE, tableKey, 0) + 1)

call SaveTriggerHandle(thistype.INIT_TABLE, tableKey, LoadInteger(thistype.INIT_TABLE, tableKey, 0), t)
//        call Memory.IntegerKeys.Table.AddInteger(INIT_PARENT_KEY_ARRAY, tableKey, t)
    endmethod

    //! runtextmacro GetKeyArray("INIT_NORMAL_KEY_ARRAY")

    static method AddObjectInitNormal takes code c returns nothing
        call thistype.AddObjectInit(INIT_NORMAL_KEY_ARRAY, c)
    endmethod

    static method RunObjectInits takes integer tableKey returns nothing
//        local integer iteration = Memory.IntegerKeys.Table.CountIntegers(INIT_PARENT_KEY_ARRAY, tableKey)
local integer count = LoadInteger(thistype.INIT_TABLE, tableKey, 0)
        local integer iteration = 1
        local trigger t

        loop
//            exitwhen (iteration == Memory.IntegerKeys.Table.EMPTY)
            exitwhen (iteration > count)

//            set this = Memory.IntegerKeys.Table.GetInteger(INIT_PARENT_KEY_ARRAY, tableKey, iteration)
            set t = LoadTriggerHandle(thistype.INIT_TABLE, tableKey, iteration)
    ...
    static method Init takes nothing returns nothing
        ...
        call thistype.RunObjectInits(INIT_NORMAL_KEY_ARRAY)
    endmethod
endstruct