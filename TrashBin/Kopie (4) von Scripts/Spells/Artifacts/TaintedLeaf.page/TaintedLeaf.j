//! runtextmacro BaseStruct("TaintedLeaf", "TAINTED_LEAF")
    ...
    static method Event_SpellEffect takes nothing returns nothing
        ...
        if (target.IsAllyOf(caster.Owner.Get())) then
            if ((targetType == HandOfNature.SUMMON_UNIT_TYPE[1]) or (targetType == HandOfNature.SUMMON_UNIT_TYPE[2]) or (targetType == HandOfNature.SUMMON_UNIT_TYPE[3]) or (targetType == HandOfNature.SUMMON_UNIT_TYPE[4]) or (targetType == HandOfNature.SUMMON_UNIT_TYPE[5]) and (target.Buffs.Contains(thistype.TAINTED_BUFF) == false)) then
