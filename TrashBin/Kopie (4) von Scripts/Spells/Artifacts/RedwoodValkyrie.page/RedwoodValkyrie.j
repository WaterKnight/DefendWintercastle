//! runtextmacro BaseStruct("RedwoodValkyrie", "REDWOOD_VALKYRIE")
    ...
    static method TargetConditions takes nothing returns boolean
        ...
        if (filterUnit.IsAllyOf(this.caster.Owner.Get()) and (filterUnit.Type.Get() != HopNDrop(NULL).SetMines.Mine.SUMMON_UNIT_TYPE)) then
            return false
        endif
    ...
    static method Impact takes nothing returns nothing
        ...
        if (target.Type.Get() == HopNDrop(NULL).SetMines.Mine.SUMMON_UNIT_TYPE) then
            call target.Kill()
